...
useEffect(()=>{
    async function get(){
setapt((await axios.get(`http://localhost:5000/patientapp?username=${id}`)).data)
console.log(apt)
    }
    get()
},[])
...
<input  type="date" onChange={(e)=>{setOld(e.target.value)}}></input>
...
{
    apt?.filter(({doctor,date})=>{
        console.log(date)
        return doctor.includes(search)&&(date===oldDate);
    }).map((data)=>
